...
cache = {}
def negamax(board, token1, token2):
    ...
    if p1:
        for i in p1:
            score, newpath = negamax(move(board, i, token1, token2), token2, token1)
            totalmoves[str(i) + ' ' + newpath] = -1 * score
    else:
        maxi, new = negamax(board, token2, token1)
        return -maxi, '-1 ' + new
