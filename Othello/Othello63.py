...
def output(board, t1, t2):
    ...
    score, path = alphabeta(puzzle, t1, t2, -64, 64, '')
...
def alphabeta(board, token1, token2, alpha, beta, path):
    ...
    if p1:
        ...
        for i in p1:
            score, newpath = alphabeta(move(board, i, token1, token2), token2, token1, -1 * beta, -1 * alpha, path + ' ' + str(i))
            score = -score
            if score > beta:
                return score, ''
            if score <= alpha:
        ...
        maxi, new = alphabeta(board, token2, token1, -1 * beta, -1 * alpha, path + ' -1')
        return -maxi, '-1 ' + new
