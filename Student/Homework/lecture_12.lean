...
-- #2. no (jam ⊕ cheese) => (no jam) x (no cheese)
def e1 := ¬(J ∨ C) ⇒ (¬J) ∧ (¬C)

-- #3. (no jam) x (no cheese) => no (jam ⊕ cheese)
def e2 := (¬J) ∧ (¬C) ⇒ ¬(J ∨ C)
...
#eval eval_expr e0 cheese_only -- expect true
#eval eval_expr e0 jam_only -- expect true
#eval eval_expr e1 cheese_only -- expect true
#eval eval_expr e1 jam_only -- expect true
#eval eval_expr e2 cheese_only -- expect true
#eval eval_expr e2 jam_only -- expect true